...
class LivePortraitPipeline(object):
    ...
    _PROPAGATED_FLAGS = (
        'flag_pasteback', 'flag_stitching', 'flag_do_crop',
        'flag_eye_retargeting', 'flag_lip_retargeting', 'flag_do_rot',
    )

    def _apply_flag_overrides(self, args: ArgumentConfig) -> None:
    ...
    def execute_streaming(self, args: ArgumentConfig, source_cache: dict | None = None):
        ...
        if flag_load_from_template:
            log(f"Load from template: {args.driving}", style='bold green')
            driving_template_dct = load(args.driving)
            c_d_eyes_lst = driving_template_dct['c_eyes_lst'] if 'c_eyes_lst' in driving_template_dct else driving_template_dct['c_d_eyes_lst']
            c_d_lip_lst = driving_template_dct['c_lip_lst'] if 'c_lip_lst' in driving_template_dct else driving_template_dct['c_d_lip_lst']
            driving_n_frames = driving_template_dct['n_frames']
            flag_is_driving_video = driving_n_frames > 1
            if flag_is_source_video and flag_is_driving_video:
            ...
            if is_video(args.driving):
                ...
                flag_is_driving_video = False
                driving_rgb_lst = [load_image_rgb(args.driving)]
                output_fps = 25
            else:
                raise Exception(f"{args.driving} is not a supported type!")
            driving_n_frames = len(driving_rgb_lst)
        ...
        if source_cache is not None and not flag_is_source_video:
            ...
            if inf_cfg.flag_do_crop:
                ...
                source_M_c2o_lst = [None] * n_frames
            c_s_eyes_lst, c_s_lip_lst = self.live_portrait_wrapper.calc_ratio(source_lmk_crop_lst)
            I_s_lst = self.live_portrait_wrapper.prepare_videos(img_crop_256x256_lst)
            source_template_dct = self.make_motion_template(I_s_lst, c_s_eyes_lst, c_s_lip_lst, output_fps=source_fps)
            key_r = 'R' if 'R' in driving_template_dct['motion'][0] else 'R_d'
            if inf_cfg.flag_relative_motion:
                if flag_is_driving_video:
                    x_d_exp_lst = [source_template_dct['motion'][i]['exp'] + driving_template_dct['motion'][i]['exp'] - driving_template_dct['motion'][0]['exp'] for i in range(n_frames)]
                    x_d_exp_lst_smooth = smooth(x_d_exp_lst, source_template_dct['motion'][0]['exp'].shape, device, inf_cfg.driving_smooth_observation_variance)
        ...
        def _generate_frames():
            ...
            for i in range(n_frames):
                ...
                if flag_is_source_video:
                    ...
                    source_lmk_i = source_lmk_crop_lst[i]
                    img_crop_256x256_i = img_crop_256x256_lst[i]
                    I_s_i = I_s_lst[i]
                    f_s_i = self.live_portrait_wrapper.extract_feature_3d(I_s_i)
                    x_c_s_i = x_s_info_i['kp']
                    R_s_i = x_s_info_i['R']
                    x_s_i = x_s_info_i['x_s']
                    if flag_normalize_lip and inf_cfg.flag_relative_motion and source_lmk_i is not None:
                        c_d_lip_before_animation = [0.]
                        combined_lip_ratio_tensor_before_animation = self.live_portrait_wrapper.calc_combined_lip_ratio(c_d_lip_before_animation, source_lmk_i)
                        if combined_lip_ratio_tensor_before_animation[0][0] >= inf_cfg.lip_normalize_threshold:
                            lip_delta_before_animation = self.live_portrait_wrapper.retarget_lip(x_s_i, combined_lip_ratio_tensor_before_animation)
                        else:
                            lip_delta_before_animation = None
                    if flag_source_video_eye_retargeting and source_lmk_i is not None:
                        if i == 0:
                            combined_eye_ratio_tensor_frame_zero = c_s_eyes_lst[0]
                            c_d_eye_before_animation_frame_zero = [[combined_eye_ratio_tensor_frame_zero[0][:2].mean()]]
                            if c_d_eye_before_animation_frame_zero[0][0] < inf_cfg.source_video_eye_retargeting_threshold:
                                c_d_eye_before_animation_frame_zero = [[0.39]]
                        combined_eye_ratio_tensor_before_animation = self.live_portrait_wrapper.calc_combined_eye_ratio(c_d_eye_before_animation_frame_zero, source_lmk_i)
                        eye_delta_before_animation = self.live_portrait_wrapper.retarget_eye(x_s_i, combined_eye_ratio_tensor_before_animation)
                    if inf_cfg.flag_pasteback and inf_cfg.flag_do_crop and inf_cfg.flag_stitching:
                        mask_ori_float = prepare_paste_back(inf_cfg.mask_crop, source_M_c2o_lst[i], dsize=(source_rgb_lst[i].shape[1], source_rgb_lst[i].shape[0]))
                    # use per-frame source variables
                    x_c_s_use, R_s_use, f_s_use, x_s_use, source_lmk_use = x_c_s_i, R_s_i, f_s_i, x_s_i, source_lmk_i
                else:
                    x_c_s_use, R_s_use, f_s_use, x_s_use, source_lmk_use = x_c_s, R_s, f_s, x_s, source_lmk
                ...
                delta_new = x_s_use['exp'].clone() if flag_is_source_video else x_s_info['exp'].clone()
                if inf_cfg.flag_relative_motion:
                    if inf_cfg.animation_region in ("all", "pose"):
                        R_new = x_d_r_lst_smooth[i] if flag_is_source_video else (R_d_i @ R_d_0.permute(0, 2, 1)) @ R_s_use
                    else:
                        R_new = R_s_use
                    if inf_cfg.animation_region in ("all", "exp"):
                        ...
                        for eyes_idx in [11, 13, 15, 16, 18]:
                            if flag_is_source_video:
                                ...
                                delta_new[:, eyes_idx, :] = (x_s_info['exp'] + (x_d_i_info['exp'] - 0))[:, eyes_idx, :]
                    if inf_cfg.animation_region == "all":
                        scale_new = x_s_use['scale'] if flag_is_source_video else x_s_info['scale'] * (x_d_i_info['scale'] / x_d_0_info['scale'])
                    else:
                        scale_new = x_s_use['scale'] if flag_is_source_video else x_s_info['scale']
                    if inf_cfg.animation_region in ("all", "pose"):
                        t_new = x_s_use['t'] if flag_is_source_video else x_s_info['t'] + (x_d_i_info['t'] - x_d_0_info['t'])
                    else:
                        t_new = x_s_use['t'] if flag_is_source_video else x_s_info['t']
                else:
                    if inf_cfg.animation_region in ("all", "pose"):
                        R_new = x_d_r_lst_smooth[i] if flag_is_source_video else R_d_i
                    else:
                ...
                t_new[..., 2].fill_(0)
                x_d_i_new = scale_new * (x_c_s_use @ R_new + delta_new) + t_new

                if inf_cfg.flag_relative_motion and inf_cfg.driving_option == "expression-friendly" and not flag_is_source_video and flag_is_driving_video:
                    if i == 0:
                        x_d_0_new = x_d_i_new
                        motion_multiplier = calc_motion_multiplier(x_s_use, x_d_0_new)
                    x_d_diff = (x_d_i_new - x_d_0_new) * motion_multiplier
                    x_d_i_new = x_d_diff + x_s_use
                ...
                if not inf_cfg.flag_stitching and not inf_cfg.flag_eye_retargeting and not inf_cfg.flag_lip_retargeting:
                    ...
                    if flag_normalize_lip and lip_delta_before_animation is not None:
                        x_d_i_new = self.live_portrait_wrapper.stitching(x_s_use, x_d_i_new) + lip_delta_before_animation
                    else:
                    ...
                    eyes_delta, lip_delta = None, None
                    if inf_cfg.flag_eye_retargeting and source_lmk_use is not None:
                        c_d_eyes_i = c_d_eyes_lst[i]
                        combined_eye_ratio_tensor = self.live_portrait_wrapper.calc_combined_eye_ratio(c_d_eyes_i, source_lmk_use)
                        eyes_delta = self.live_portrait_wrapper.retarget_eye(x_s_use, combined_eye_ratio_tensor)
                    if inf_cfg.flag_lip_retargeting and source_lmk_use is not None:
                        c_d_lip_i = c_d_lip_lst[i]
                        combined_lip_ratio_tensor = self.live_portrait_wrapper.calc_combined_lip_ratio(c_d_lip_i, source_lmk_use)
                        lip_delta = self.live_portrait_wrapper.retarget_lip(x_s_use, combined_lip_ratio_tensor)
                    if inf_cfg.flag_relative_motion:
                        x_d_i_new = x_s_use + \
                            (eyes_delta if eyes_delta is not None else 0) + \
                            (lip_delta if lip_delta is not None else 0)
